...
| Snowman workforce identity authority | Workforce login, cryptographic identity binding, and Snowman-managed community lifecycle | Analyst 360 now implements the initial privacy-preserving Google Workspace/KMS enrollment authority in source; it is private, default-off, and not deployed. Inherited Builderlab/Block identity endpoints remain prohibited. | Short-lived, tenant-scoped KMS assertions; pseudonymous subject digest; Nostr device proof; no provider token/email/raw subject; current MFA-policy evidence; revocation, recovery, private-routing, and staging evidence remain required. |
