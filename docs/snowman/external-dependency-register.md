...
| Snowman workforce identity authority | Workforce login, cryptographic identity binding, and Snowman-managed community lifecycle | Analyst 360 implements privacy-preserving Google Workspace/KMS enrollment and signed session/global/identity revocation in source; it is private, default-off, and not deployed. Inherited Builderlab/Block identity endpoints remain prohibited. | Short-lived, tenant-scoped KMS assertions; pseudonymous subject digest; Nostr device proof; no provider token/email/raw subject; current MFA-policy evidence; durable retry/recovery, private-routing, and staging evidence remain required. |
...
| Snowman durable agent runtime | Run scheduled and proactive specialist work safely when user devices are offline | PostgreSQL queue/leases, identity-isolated workers, v2 proactive-to-task materialization, an idempotent deadline/recovery scheduler, a separately scoped recurring trigger, and a recipient/message-denying in-product reminder worker plus hard-dormant AWS isolation now exist in source. Optional calendar delivery, broader sandbox/credential broker, activation, and staged proof remain required. | Tenant-bound queues/workers, lease/heartbeat, deterministic schedule claims and reminder receipts, idempotency, sandbox, recovery, dead-letter policy, cost ceilings, audit correlation, dormant staging controls. |
