...
| Snowman model gateway and inference fleet | Enforce per-agent best-fit model policy without exposing vendor endpoints or credentials to clients/workers | The locally tested gateway now supports only private Snowman origins or exact same-account SageMaker endpoints through private AWS networking. The fleet is required and not yet deployed/proven; zero-third-party mode uses models hosted in Snowman AWS. | Tenant/data-class policy, exact endpoint IAM, VPC endpoint path, per-route receipts, model catalog, evaluation results, budgets, throttles, kill switch, negative direct-egress test. |
| Snowman durable agent runtime | Run scheduled and proactive specialist work safely when user devices are offline | PostgreSQL queue/leases and the identity-isolated claim/plan/heartbeat/Analyst/context/completion worker now exist in source; capability-specific Analyst executors, scheduler, sandbox, credential broker, deployment, and proof remain required | Tenant-bound queues/workers, lease/heartbeat, idempotency, sandbox, scheduler, recovery, dead-letter policy, cost ceilings, audit correlation, dormant staging controls. |
