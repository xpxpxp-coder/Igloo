...
| Snowman Google Workspace | Human SSO, lifecycle, and governed per-tenant meeting mailbox/calendar identities | Approved Snowman ecosystem; dedicated user accounts are required where the agent must receive mail, invitations, and calendar conference data because aliases are not accounts. | Stable subject, tenant/role claims, auth strength, exact least-privilege Gmail/Calendar scopes, Pub/Sub watch renewal/history reconciliation, per-restricted-tenant mailbox isolation, revocation/audit evidence, and bounded context delivery; no provider token in any agent/client/relay and no raw client work product outside Analyst authority. |
...
| OpenAI Realtime API | Optional low-latency conversational model for eligible Snowman huddles and phone sessions | User-authorized for default-off design; not approved for Aptive/restricted audio or production activation until the processing boundary and staged evidence pass. | Live audio may contain unexpected PII before redaction. Require exact API/WebSocket/SIP hosts, Snowman-held credential, meeting classification/consent, retention/training/region evidence, no raw storage by default, transcript/output receipts, duration/spend limits, kill switch, and zero direct client/agent credentials. |
| Snowman Twilio account | Optional PSTN/SIP number, inbound webhooks, and narrowly controlled outbound meeting dialing | User-authorized for default-off design; activation remains meeting-policy and evidence gated. AWS Chime SDK remains the Snowman-ecosystem alternative. | Exact Snowman account/subaccount, verified webhook signatures and replay fence, allowlisted caller/dial targets and SIP routes, deterministic DTMF, recordings disabled by default, phone identifiers minimized/hashed, regional/retention evidence, spend/fraud limits, and immediate hangup switch. |
| ElevenLabs | Optional premium TTS for an explicitly chained meeting route | User-authorized for default-off design; not used for inbound audio or the default OpenAI speech-to-speech route. | Approved response text only; no inbound audio, raw transcript, Aptive evidence, credentials, or tool context. Require voice/model allowlist, retention/training/region evidence, text/output digest receipts, cost ceiling, and kill switch. |
...
| Snowman durable agent runtime | Run scheduled and proactive specialist work safely when user devices are offline | PostgreSQL queue/leases, identity-isolated workers, proactive materialization/scheduling, credentialless one-shot Fargate task definitions, strict job contracts, the private broker, and a NIP-98/service-key-bound crash-fenced coordinator with running-task reconciliation, separate KMS-MACed model grants, and source-IP-attested private credential bootstrap now exist in source. Worker-to-coordinator wiring, the executor-local model proxy/live spend checks, action-tool paths, optional adapter images, activation, and staged proof remain required. | Tenant-bound queues/workers, lease/heartbeat, deterministic claims, idempotency, no executor task role, pinned runtime/SBOM/provenance/evaluation digests, deny-by-default private egress, PII-prohibited minimized snapshots, no bearer secrets in ECS launch metadata, NLB-preserved task-IP/ECS attachment attestation, constant-time token-digest verification, exact launch/bootstrap/broker receipts, recovery, retention/purge, dead-letter policy, cost ceilings, audit correlation, dormant staging controls. |
