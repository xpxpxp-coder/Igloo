...
an empty KMS-encrypted Secrets Manager shell and two dormant task definitions:
...
- `snowman-bootstrap` is a one-shot task with no service. It can read the exact
  ...
  private database, and write migration logs. It has no NAT or public route.
...
8. connects as the runtime identity and fails unless required tables and DML
   privileges exist and DDL privileges do not; and
9. writes the exact four-field JSON document to the governed runtime secret and
...
not a generic deployment default: it must be the public key enrolled to the
Snowman workforce owner identity. Supply it as a reviewed ECS RunTask container
environment override only after workforce enrollment. The task fails closed when
