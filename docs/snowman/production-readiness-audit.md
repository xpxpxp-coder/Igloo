...
| Audit chain | External checkpoint implementation complete in source; staged proof required | `crates/buzz-audit/src/{hash,service}.rs`, `crates/snowman-audit-checkpoint`, migration 0054, and `infra/aws/audit_checkpoint.tf` verify exact tenant segments, link KMS-signed envelopes, and retain them under S3 Object Lock independently of DB receipts | Keep the schedule disabled until KMS rotation, immutable S3, cross-tenant, lost-response, tamper, alert, PITR/restore, and launch-evidence drills pass. |
