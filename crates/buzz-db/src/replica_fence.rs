...
mod tests {
    ...
    fn fence_starts_closed_and_opens_on_advance() {
        ...
        // ReplicaFence intentionally stores Unix microseconds, matching the
        // PostgreSQL timestamp precision used by the fence proof.
        let ts = DateTime::from_timestamp_micros(Utc::now().timestamp_micros())
            .expect("current timestamp fits Unix microseconds");
        fence.advance(ts);
