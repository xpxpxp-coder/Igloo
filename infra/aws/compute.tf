...
resource "aws_secretsmanager_secret" "agent_coordinator_runtime" {
  name                    = "/snowman/command-center/${var.environment}/agent-coordinator-runtime"
  description             = "Private agent coordinator database URL populated only by the governed bootstrap"
  kms_key_id              = aws_kms_key.data.arn
  recovery_window_in_days = 30
}

resource "aws_secretsmanager_secret" "model_gateway_runtime" {
  name                    = "/snowman/command-center/${var.environment}/model-gateway-runtime"
  description             = "Private model gateway database URL populated only by the governed bootstrap"
  kms_key_id              = aws_kms_key.data.arn
  recovery_window_in_days = 30
}
...
data "aws_iam_policy_document" "bootstrap_task" {
  ...
  statement {
    ...
    resources = [
      ...
      aws_secretsmanager_secret.meeting_media_runtime.arn,
      aws_secretsmanager_secret.provider_egress_runtime.arn,
    ]
  }
  dynamic "statement" {
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "SNOWMAN_AGENT_COORDINATOR_RUNTIME_SECRET_ARN", value = aws_secretsmanager_secret.agent_coordinator_runtime.arn },
        ...
        { name = "SNOWMAN_MEETING_MEDIA_RUNTIME_SECRET_ARN", value = aws_secretsmanager_secret.meeting_media_runtime.arn },
        { name = "SNOWMAN_MEETING_MEDIA_DB_ROLE", value = "snowman_meeting_media" },
        { name = "SNOWMAN_PROVIDER_EGRESS_RUNTIME_SECRET_ARN", value = aws_secretsmanager_secret.provider_egress_runtime.arn },
        { name = "SNOWMAN_PROVIDER_EGRESS_DB_ROLE", value = "snowman_provider_egress" },
