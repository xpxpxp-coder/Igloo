...
function buildMockConfigSurface(pubkey: string): {
  ...
  // panel witnesses distinct provenance labels in one frame: "Set in Snowman Command Center",
  ...
  const defaultCatalog: RawAcpRuntimeCatalogEntry[] = [
    {
      ...
      default_args: ["acp"],
      mcp_command: null,
      install_hint: "Use the Snowman-managed agent runtime package.",
      install_instructions_url: "https://snowmanai.org/docs/agents",
...
async function handleStartManagedAgent(
  ...
  if (isRelayMeshManagedAgent(agent)) {
    ...
    if (!hasLiveTarget) {
      throw new Error(
        "Snowman shared compute cannot start because no live member is serving this model.",
      );
    }
  }
      ...
      case "discover_agent_models": {
        ...
        if (provider === "relay-mesh") {
          ...
          if (mockMeshState.models.length === 0) {
            throw new Error(
              "no Snowman shared compute serving members are available",
            );
          }
        }
        const models =
