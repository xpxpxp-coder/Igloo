...
pub fn availability_from_events(events: Vec<nostr::Event>) -> MeshAvailability {
    if events.is_empty() {
        return MeshAvailability::unavailable("Snowman shared compute status is not published yet");
    }
    let Some(members) = latest_membership_list(&events) else {
    ...
    if !saw_valid_status {
        return MeshAvailability::unavailable("Snowman shared compute status is malformed");
    }
    ...
    MeshAvailability {
        reason: if available {
            ...
            Some("no Snowman shared compute serving members are available".to_string())
        },
        models,
        serve_targets,
    }
